...
use super::types::{RouteNode, RoutedPip};
type RouteWireKey = RouteNode;

/// Per-resource claim bookkeeping for negotiated congestion.
///
...
impl ResourceClaims {
    pub(super) fn new(net_index: usize) -> Self {
        Self {
            owner: net_index,
            others: 0,
        }
    ...
    pub(super) fn claim(&mut self, net_index: usize) {
        if self.owner != net_index {
            self.others += 1;
        }
    }
...
pub(super) fn reserve_route_sinks(
    occupied_route_sinks: &mut HashMap<RouteWireKey, ResourceClaims>,
    net_index: usize,
    _origin: NetOrigin,
    path: &[RoutedPip],
) {
    for pip in path {
        occupied_route_sinks
            ...
            .and_modify(|claims| claims.claim(net_index))
            .or_insert_with(|| ResourceClaims::new(net_index));
    }
}
...
pub(super) fn count_contested<K>(
    claims: &HashMap<K, ResourceClaims>,
) -> impl Iterator<Item = (&K, &ResourceClaims)> {
    claims.iter().filter(|(_, c)| c.others > 0)
}
...
pub(super) struct NegotiationContext<'a> {
    pub(super) occupied_route_sinks: &'a HashMap<RouteNode, ResourceClaims>,
...
impl NegotiationContext<'_> {
    ...
    pub(super) fn sink_penalty(&self, key: &RouteNode) -> usize {
        self.occupied_route_sinks.get(key).map_or_else(
            || unclaimed_penalty(history_of(self.sink_history, key)),
            |claims| {
                claims.congestion_penalty(
                    self.net_index,
                    history_of(self.sink_history, key),
                    self.present_factor,
                )
            },
        )
